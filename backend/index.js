...
// SCRAPING
const getRealEstate = (n, locationCode) => {
  const pages = [];
  for (let i = 0; i < n; i++) {
    let url = locationCode
      ? `https://www.finn.no/realestate/lettings/search.html?location=${locationCode}&page=${
          i + 1
        }&sort=PUBLISHED_DESC`
    ...
    axios.get(url).then((response) => {
      ...
      $(
        ...
        const area = getKeyByValue(areas, locationCode);
...
// MAJOR CITIES / AREAS - BASED ON ID
for (const area in areas) {
  const temp = getRealEstate(N, areas[area]);
  app.get(`/${area}/:id`, async (req, res) => {
    ...
    res.json({ area, ad: correctAd[0] });
  });
}

// INDEX
// const pages = getRealEstate(N, areas.oslo);
// app.get("/", (req, res) => {
...
//     res.json({ ad: correctAd });
//   } catch (err) {
//     res.json({ message: `did not find ${id}` });
//   }
// });
